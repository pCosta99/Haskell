...
m_grd :: Monad m => (a -> m Bool) -> a -> m (Either a a)
m_grd p x = p x >>= return . flip grd x . const
...
rsplit f g = rstr . split f g
lsplit f g = lstr . split f g
pow2 f = f >< f
snoc a s = s ++ a

-- Infix operators for pointfree usage. 
infix 5 <$.>
