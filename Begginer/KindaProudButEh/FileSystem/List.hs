...
---- Section made for personal use by Pedro Costa, not directly related with the above ---------------

manaList :: Strong m => (a -> m (Either () (b,a))) -> a -> m [b]
manaList g = (fmap inList . dl . recList (manaList g)) .! g

hyloListM f g = mcataList f .! manaList g

dl2 :: Monad m => Either b (m a) -> m (Either b a)
dl2 = either (return . i1) (fmap i2)