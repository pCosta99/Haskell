...
\begin{code}
prop_avg :: (Fractional a, Ord a) => [a] -> Property
...
outExpAr (Bin bop e1 e2) = i2 $ i2 $ i1 (bop,(e1,e2))
outExpAr (Un uop e) = i2 $ i2 $ i2 (uop,e)
---
recExpAr f = baseExpAr id id id f f id f
---
g_eval_exp x = either (const x) (either id (either biop uop)) where
    biop (Sum,(x,y)) = x + y
    biop (Product,(x,y)) = x * y
    uop (Negate,x) = x * (-1)
    uop (E,x) = Prelude.exp x

---
-- O cata n conseguia parar "a meio" do algoritmo. O anamorfismo corta a execução pois consegue alterar a estrutura sobre a qual está a operar (é top to bottom).
clean :: (Floating a, Eq a) => ExpAr a -> OutExpAr a
clean v@(Bin Product (N x) _) = if x == 0 then i2 $ i1 $ 0 else outExpAr v
clean v@(Bin Product _ (N y)) = if y == 0 then i2 $ i1 $ 0 else outExpAr v
clean v = outExpAr v
---
...
sd_gen = either x $ either n $ either bop uop where
    x = const (X, N 1)
    n a = (N a, N 0)
    bop (Sum, ((e1,e2),(e3,e4))) = (Bin Sum e1 e3, Bin Sum e2 e4)
    bop (Product, ((e1,e2),(e3,e4))) = (Bin Product e1 e3, Bin Sum (Bin Product e1 e4) (Bin Product e2 e3))
    uop (Negate, (e1,e2)) = (Un Negate e1, Un Negate e2)
    uop (E, (e1,e2)) = (Un E e1, Bin Product (Un E e1) e2)
\end{code}

\begin{code}
ad_gen v = either x $ either n $ either bop uop where
    x = const (v, 1)
    n a = (a, 0)
    bop (Sum, ((e1,e2),(e3,e4))) = (e1 + e3, e2 + e4)
    bop (Product, ((e1,e2),(e3,e4))) = (e1 * e3, (e1*e4) + (e2*e3))
    uop (Negate, (e1,e2)) = ((-1) * e1, (-1) * e2)
    uop (E, (e1,e2)) = (expd e1, expd e1 * e2)
\end{code}

\subsection*{Problema 2}
Definir
\begin{code}
loop (c,h,s,t,f) = (div (c * h) s, h + t, s + f, t + 8, f + 2)
inic = (1,2,2,10,4)
prj (c,_,_,_,_) = c
\end{code}
por forma a que
...
cat2 = (\(c,_,_,_,_) -> c) . (for loop inic)
\end{code}
...
\begin{code}
calcLine :: NPoint -> (NPoint -> OverTime NPoint)
calcLine = cataList $ either f g where
   f = const $ const nil
   g (q,k) l f = if null l then k l f else linear1d q (head l) f : k (tail l) f

calcLine2 :: NPoint -> (NPoint -> OverTime NPoint)
calcLine2 = cataList $ either f g where
    f = const $ const nil
    g (q,k) l f = linear1d q (l !! (length l - length (k l f) - 1)) f : k l f
...
\begin{code}
avg_aux :: (Num a, Fractional a) => [a] -> (a,a)
avg_aux = cataList avg_gene where
    avg_gene = either (split (const 0) (const 0)) (\(x,(a,k)) -> let l = k+1 in ((x+(a*k))/l, l))
\end{code}
Solução para árvores de tipo \LTree:
\begin{code}
avgLTree :: (Num a, Fractional a) => LTree a -> a
avgLTree = p1 . cataLTree gene2 where
   gene2 = either (split id (const 1)) (\((x1,y1),(x2,y2)) -> let l = y1+y2 in ((x1*y1 + x2*y2)/l,l))
\end{code}
